...
// Temporary fallback stub for AI prediction until aiPredictionService is fully built
async function predictWaitTime(
  queuePosition,
  avgConsultationMins,
  queueLength,
) {
  return null; // Returning null safely triggers the math fallback formula below
}
...
async function recalculateDepartmentQueue(departmentCode) {
  ...
  const inConsultationCount = await Token.countDocuments({
    departmentCode,
    status: "In Consultation",
  });

  const doctorLoadFactor = Math.max(
    department.activeDoctors - inConsultationCount,
    1,
  );
  ...
  const predictions = await Promise.all(
    waitingTokens.map((_, index) =>
      predictWaitTime(index + 1, department.avgConsultationMins, queueLength),
    ),
  );
  ...
  const bulkOps = waitingTokens.map((token, index) => {
    ...
    const formulaWaitMins = Math.max(
      1,
      Math.round(
        (department.avgConsultationMins * queuePosition * delayFactor) /
          doctorLoadFactor,
      ),
    );

    // Use AI prediction if available, otherwise use formula
    const predictedWaitMins =
      predictions[index]?.estimatedWaitTime ?? formulaWaitMins;
...
async function getQueueOverview() {
  ...
  for (const department of departments) {
    ...
    const waitingQueue = await Token.find({
      departmentCode: department.code,
      status: "Waiting",
    })
      .sort({ createdAt: 1 })
      .select(
        "tokenId patientName status queuePosition predictedWaitMins predictionSource assignedDoctor assignedRoom",
      );
...
async function getDashboardStats() {
  const [waitingCount, inConsultationCount, completedTokens, waitingTokens] =
    await Promise.all([
    ...
    completedTokens.length > 0
      ? Math.round(
          completedTokens.reduce((sum, t) => {
            const duration =
              (new Date(t.consultationCompletedAt) -
                new Date(t.consultationStartedAt)) /
              60000;
    ...
    waitingTokens.length > 0
      ? Math.round(
          waitingTokens.reduce(
            (sum, t) => sum + (t.predictedWaitMins || 0),
            0,
